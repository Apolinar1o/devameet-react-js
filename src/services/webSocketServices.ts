...
class PeerConnectionSession {
    ...
    addPeerConnection(id: string, stream: any, callback: any) {
        if(!this.peerConnections[id]) {
            ...
            this.peerConnections[id].addEventListener("connectionsStateChange", this.listener[id])
    ...
    removePeerConnection(id: string) {
        if(!this.peerConnections[id]) {
            this.peerConnections[id].removeEventListener("connectionsStateChange", this.listener[id])
    ...
    async callUser(to: any) {
        console.log("callUSer: ", to, this.peerConnections[to].iceConnectionState)
        if(this.peerConnections[to].iceConnectionState === "new") {
            ...
            this.socket.mit("call-user", {offer, to, link: this._room})
        }
    }
   ...
   async onCallMade() {
        this.socket.on(`call-made`, async (data : any) => {
            ...
            if(selectedPeer) {
                ...
                this.socket.mit("make-answer", {
                    answer, to: data.socket, link: this._room
                })
            }
